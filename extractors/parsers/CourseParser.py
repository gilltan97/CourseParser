from extractors.utils import Scraper
from extractors.parsers import Helpers
...
class CourseParser:
    ...
    def parse_html(self):
        ...
        return {
            'course code': self.course_code[:-5],
            'course name': Helpers.get_name(soup),
            'course division': Helpers.get_division(soup),
            'course department': Helpers.get_department(soup),
            'course prerequisites': Helpers.get_prerequisites(soup),
            'course exclusion': Helpers.get_exclusion(soup),
            'course level': Helpers.get_courselevel(soup),
